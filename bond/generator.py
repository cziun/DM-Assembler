...
class Generator(nn.Module):
    def __init__(self, tokenizer, atom_embedding_dim, piece_embedding_dim, max_pos,
                 pos_embedding_dim, num_edge_type, node_hidden_dim, t=4):
    ...
    def forward(self, x, edge_index, edge_attr, pieces, edge_select, golden_edge, return_accu=False):
    ...
    def inference(self, motif_ids, adj_inter_motif, add_edge_th, device):
        ...
        src_embedding = torch.repeat_interleave(node_embedding, node_num, dim=1).view(1, node_num, node_num, -1)
        src_embedding = src_embedding[edge_select]  
        dst_embedding = torch.repeat_interleave(node_embedding, node_num, dim=0).view(1, node_num, node_num, -1)
        dst_embedding = dst_embedding[edge_select]  
        edge_pred_in = torch.cat([src_embedding, dst_embedding], dim=-1)
