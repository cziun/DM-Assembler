...
def train(model, train_loader, valid_loader, test_loader, config, args):
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        dirpath=f"checkpoints/BondRecovery/{config.data.data}/{current_time}",
        filename='best_model',
        mode='min', 
        save_top_k=1,
        verbose=True
    ...
    trainer_config = {
        ...
        'default_root_dir': f"checkpoints/BondRecovery/{config.data.data}/{current_time}",
...
def parse():
    ...
    parser.add_argument('--train_set', type=str, default='data/dtp/train_props.csv')
    parser.add_argument('--valid_set', type=str, default='data/dtp/valid_props.csv')
    parser.add_argument('--test_set', type=str, default='data/dtp/test_props.csv')
...
if __name__ == '__main__':
    ...
    model = BondRecovery(config, tokenizer)
    # Train
